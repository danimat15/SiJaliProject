...
class WordCloudSetting {
    ...
    for (var i = 1; i < textlist.length; i++) {
      ...
      while (!isadded) {
        ...
        if (attempts > maxAttempts) {
          // Check against the increased attempt limit
          isadded = true;
          checkattempt = true;
        }
      }
  ...
  void drawText() {
    ...
    for (var i = 1; i < textlist.length; i++) {
      ...
      for (int y = 0; y < mapY.toInt(); y++) {
        for (int x = 0; x < mapX.toInt(); x++) {
          if (checkMap(x.toDouble(), y.toDouble(), w, h)) {
            drawIn(i, x.toDouble(), y.toDouble());
            isadded = true;
            break;
          }
        }
